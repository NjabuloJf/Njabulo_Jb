...
const generateReactionCommand = (reactionName, reactionEmoji) => {
  fana({
    ...
    try {
      ...
      const videoBuffer2 = await GIFBufferToVideoBuffer(gifBuffer);
      // Envoyer la vidéo avec fana
      const videoMessage1 = (await zk.generateWAMessageContent({ video: videoBuffer, gifPlayback: true }, { upload: zk.waUploadToServer })).videoMessage;
      const videoMessage2 = (await zk.generateWAMessageContent({ video: videoBuffer2, gifPlayback: true }, { upload: zk.waUploadToServer })).videoMessage;

      const message = zk.generateWAMessageFromContent(
        origineMessage,
        {
          viewOnceMessage: {
            message: {
              ...
              interactiveMessage: {
                ...
                carouselMessage: {
                  cards: [
                    {
                      header: {
                        title: `${reactionName} Reaction 1`,
                        hasMediaAttachment: true,
                        videoMessage: videoMessage1,
                      },
                      body: {
                        text: `Check out this ${reactionName} reaction!`,
                      },
                      footer: {
                        text: "LUCKY MD",
                      },
