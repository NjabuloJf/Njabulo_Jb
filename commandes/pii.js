...
fana(
  ...
  async (dest, zk, commandeOptions) => {
    const { repondre, ms, arg, client } = commandeOptions;
    if (!arg[0]) {
    ...
    try {
      ...
      const cards = await Promise.all(
        validImages.map(async (item, i) => ({
          header: {
            ...
            imageMessage: (await generateWAMessageContent({ image: item.buffer }, { upload: client.waUploadToServer })).imageMessage,
          },
          body: { text: `🔍 Search: ${q}` },
      ...
      await client.relayMessage(dest, message.message, { messageId: message.key.id });
      await zk.deleteMessage(dest, loadingMessage.key);
    } catch (error) {
