...
async function addOrUpdateDataInMention(url, type, message) {
  try {
    ...
    try {
      const query = `
        INSERT INTO mention (id, url, type, message) VALUES (1, $1, $2, $3) 
        ON CONFLICT (id) DO UPDATE SET url = excluded.url, type = excluded.type, message = excluded.message;
      `;
...
async function modifierStatusId1(nouveauStatus) {
  try {
    ...
    try {
      const query = `
        UPDATE mention SET status = $1 WHERE id = 1;
      `;
